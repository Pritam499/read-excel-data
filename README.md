...
* `TableNotFound(name) → HTTPException 404` with message
  `“Table ‘name’ not found”`
* `RowNotFound(table, row) → HTTPException 404` with message
  `“Row ‘row’ not found in table ‘table’”`
...
* **`TableDetails`**:
  ...
  ```json
  { "table_name": str; "row_names": List[str] }
  ```
* **`RowSum`**:

  ```json
  { "table_name": str; "row_name": str; "sum": float }
  ```
* **`ErrorResponse`**:
...
* Creates a **`FastAPI()`** instance titled “Read Excel Data API”
...
* “Uvicorn running on [http://0.0.0.0:9090”](http://0.0.0.0:9090”)
